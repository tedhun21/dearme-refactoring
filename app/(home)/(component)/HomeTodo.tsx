    ...
    <section className="mt-4">
      {isLogin ? (
        <Link href={`/${dayjs(date).format("YYYY-MM-DD")}/todogoal`}>
          <div className="group flex flex-col rounded-xl border-2 border-default-300 bg-default-100 p-3 text-xl text-default-800 shadow-xl transition-colors duration-200 hover:border-default-400 hover:bg-default-800">
            ...
            <div className="flex justify-center p-6">
              {Array.isArray(todos) && todos.length > 0 ? (
                ...
                >
                  <div className="text-default-800 group-hover:text-default-900">
                    {value}%
                  </div>
                </CircularProgress>
              ) : (
